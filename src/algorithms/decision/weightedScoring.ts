...
/**
 * Multi-Criteria Weighted Scoring (AHP / Weighted Sum Model)
 * Time Complexity: O(N * K) where N = orders, K = criteria count
...
export function runWeightedScoring(
  ...
  const scores: OrderDecisionScore[] = orders.map(order => {
    // 1. Urgency: shorter deadline => higher score (0-100)
    const urgencyScore = Math.max(0, Math.min(100, 100 - (order.deadlineHours / 12) * 100));

    // 2. Customer Tier (Platinum: 100, Gold: 70, Standard: 35)
    const tierScore = order.customerTier === 'Platinum' ? 100 : order.customerTier === 'Gold' ? 70 : 35;

    // 3. Item Economic Value (Normalized up to $30,000)
    ...
    // 5. Fragility
    const fragilityScore = order.fragility === 'High' ? 100 : order.fragility === 'Medium' ? 60 : 20;
    ...
    const explanations: DecisionExplanation[] = [
      {
        ...
        weightedContribution: Math.round(urgencyScore * normalizedWeights.urgency * 10) / 10,
        interpretation: `Deadline of ${order.deadlineHours}h generates ${urgencyScore > 75 ? 'Critical rush' : 'Standard dispatch'} window.`
      },
      {
        factor: 'Customer Tier SLA',
        rawScore: tierScore,
        weight: Math.round(normalizedWeights.tier * 100) / 100,
        weightedContribution: Math.round(tierScore * normalizedWeights.tier * 10) / 10,
      ...
      },
      {
        ...
        weightedContribution: Math.round(valueScore * normalizedWeights.value * 10) / 10,
      ...
      },
      {
        ...
        weightedContribution: Math.round(perishabilityScore * normalizedWeights.perishability * 10) / 10,
        interpretation: order.isPerishable ? 'Perishable freight requires cold-chain / expedited transit.' : 'Non-perishable ambient load.'
      },
      {
        factor: 'Handling Fragility',
        rawScore: fragilityScore,
        weight: Math.round(normalizedWeights.fragility * 100) / 100,
        weightedContribution: Math.round(fragilityScore * normalizedWeights.fragility * 10) / 10,
        interpretation: `${order.fragility} fragility classification requiring specialized handling.`
      }
    ];

    const compositeScore = Math.round(
      explanations.reduce((sum, e) => sum + e.weightedContribution, 0) * 10
    ) / 10;
    ...
    if (compositeScore >= 80) {
      ...
      recommendedVehicleType = order.weightKg < 180 ? 'Electric_Cargo_Bike' : 'Van';
      ...
      recommendedVehicleType = order.weightKg > 2000 ? 'Heavy_Lorry' : 'Truck';
      recommendedTimeSlot = 'Slot C: Afternoon Window (12:00 - 16:00)';
    }

    const rulesTriggered: string[] = [];
    if (order.isPerishable && order.deadlineHours <= 3) {
      rulesTriggered.push('RULE-EXPEDITED-COLDCHAIN: Priority route assignment with temperature logging');
    }
    if (order.customerTier === 'Platinum') {
      rulesTriggered.push('RULE-VIP-SLA: Guaranteed delivery within contract time-window');
    }
    if (order.itemValue > 15000) {
      rulesTriggered.push('RULE-HIGH-VALUE-CARGO: Dedicated tracking and senior driver assignment');
    }
    if (order.weightKg > 2000) {
      rulesTriggered.push('RULE-HEAVY-FREIGHT: Dual-axle vehicle dispatch required');
    }
